...
class dict_model(pl.LightningModule):
    def __init__(self, features_dim=23, time_limit=0, start_time=None,
                 num_layers=10, hidden_dim=64, **kwargs):
        super().__init__()

        self.fc_test = nn.Linear(features_dim, hidden_dim)
        self.fclayers = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for i in range(num_layers)] + [nn.Linear(64, 1)])
    ...
    def network_step(self, g):
        ...
        score_mask = g['y_mask'][0]
        x = torch.cat([one_hot, features], dim=1)
        x = self.fc_test(x)
        for il in self.fclayers:
            x = il(x)
...
class graph_model(pl.LightningModule):
    def __init__(self, features_dim=23, time_limit=0, start_time=None, **kwargs):
        super().__init__()
